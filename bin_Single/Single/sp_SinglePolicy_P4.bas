Attribute VB_Name = "sp_SinglePolicy_P4"
Public Sub p_SinglePolicy_P4()
...
wb(2).Activate
   ...
   Set sourceDoc = wb(2).Sheets("Single Policy Inputs").Range("E5")
...
'Open new DataSet4 document and set to active
Workbooks.Add
Set wb(1) = ActiveWorkbook
ActiveWindow.WindowState = xlMaximized

'Rename worksheet to DataSet4
Sheets("Sheet1").Name = "DataSet4"

wb(1).Activate

 On Error Resume Next

    Set sourceDoc = wb(2).Sheets("Single Policy Inputs").Range("C5")
For Each sourceRng(2) In sourceDoc
 ...
 Next sourceRng(2)
     ...
     Set sourceDoc = wb(2).Sheets("Single Policy Inputs").Range("G5")
For Each sourceRng(4) In sourceDoc
 ...
 Next sourceRng(4)
    ...
    Set sourceDoc = wb(2).Sheets("Single Policy Inputs").Range("H5:I5")
For Each sourceRng(5) In sourceDoc
 ...
 Next sourceRng(5)
    ...
    Set sourceDoc = wb(2).Sheets("Single Policy Inputs").Range("J5")
...
s = wb(2).Sheets("Single Policy Inputs").Range("C5").Value
c = wb(2).Sheets("Single Policy Inputs").Range("D5").Value
tr = wb(2).Sheets("Single Policy Inputs").Range("F5").Value
ad = wb(2).Sheets("Single Policy Inputs").Range("G5").Value
ll = wb(2).Sheets("Single Policy Inputs").Range("H5").Value
ul = wb(2).Sheets("Single Policy Inputs").Range("I5").Value
pl = wb(2).Sheets("Single Policy Inputs").Range("J5").Value
t = wb(2).Sheets("Single Policy Inputs").Range("K5").Value
...
Set adoDbRs(1) = adoDbConn(1).Execute("Declare @n VARCHAR(50) Declare @l Decimal(18,2) Declare @cl Decimal(18,2) Declare @ed Date Declare @tr VARCHAR(20) Declare @cc VARCHAR(4) select top 10 o.StateCode, o.CountyCode, o.OrderNumber, p.Trancode, p.EffectiveDate, p.Liability, p.CreditLiability from Orders o inner join Policies p on p.OrderId = o.Id inner join OrderTags ot on o.Id = ot.Order_Id inner join tags t on ot.Tag_Id = t.Id where o.StateCode = '" & s & "' and o.CountyCode like isnull(@cc, '%" & c & "%') and p.TranCode like isnull(@tr,'%" & tr & "%') and p.EffectiveDate >= isnull(@ed,'" & ad & "') and p.Liability >= isnull(@l,'" & ll & "') and p.Liability <= isnull(@l, '" & ul & "') and p.CreditLiability >= isnull(@cl,'" & pl & "') and t.Name like isnull(@n, '%" & t & "%') and o.OrderNumber in (SELECT OrderNumber FROM Orders GROUP BY OrderNumber HAVING COUNT(OrderNumber)=1) Order By o.OrderNumber asc")
  ...
  'Transfer result.
    On Error Resume Next
  wb(1).Sheets("DataSet4").Range("B2:H2").CopyFromRecordset adoDbRs(1)
  'Close the recordset
  adoDbRs(1).Close
End If
...
wb(1).Sheets("DataSet4").Range("I2").Value = "AgencyNumber"
wb(2).Sheets("Single Policy Inputs").Range("B5").Copy

With wb(1).Sheets("DataSet4")
...
LastRow = wb(1).Sheets("DataSet4").Range("B" & Rows.Count).End(xlUp).Row
...
wb(1).SaveAs Filename:="H:\ORT Projects\Rate Engine Rewrite\VBA Macros\Data_Processing\File4.xlsx"
